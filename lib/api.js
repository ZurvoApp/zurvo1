...
export async function getTripPositions(tripId) {
  const { data, error } = await requireDb()
    .from('ride_positions')
    .select('rider_id, lat, lng, speed_kmh, heading, status, updated_at, rider:profiles!rider_id(id,name,initials,tint,bike)')
    .eq('trip_id', tripId)
  if (error) throw error
  return (data ?? []).map((p) => ({
    riderId: p.rider_id,
...
export async function sharePosition(tripId, { lat, lng, speedKmh = 0, heading = null, status = 'riding' }) {
  const id = await myId()
...
/* Change just my status on the pin I'm already sharing — refuelling, waiting,
   fell behind. It's an UPDATE, not an upsert: you set a status while live, so the
   row exists; touching updated_at keeps my pin from being aged out as stale. */
export async function setRideStatus(tripId, status) {
  const id = await myId()
