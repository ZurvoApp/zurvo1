...
const FEATURES = [
  { icon: '🛡️', title: 'Verified organisers only', body: 'Every ride is run by someone Zurvo has ID-checked. No strangers leading you into the hills.' },
  { icon: '🔒', title: 'Your money sits in escrow', body: 'You pay Zurvo, not the organiser. It’s released only after you finish the ride.' },
  { icon: '📍', title: 'Live map while you ride', body: 'See every rider on your trip move in real time — their location and speed, the whole way.' },
  { icon: '🤝', title: 'Ride with people you know', body: 'See which riders you’ve ridden with before on every trip, before you commit.' },
]
  ...
  return (
    <main className={styles.wrap}>
      <section className={styles.hero}>
        <img className={styles.logo} src="/icon.png" alt="Zurvo" width={84} height={84} />
        <h1 className={styles.name}>Zurvo</h1>
        <p className={styles.tag}>Ride with people. Not with strangers.</p>
        <p className={styles.sub}>Group motorcycle rides across India — with verified organisers and your money held safe until you finish.</p>

        <div className={styles.cta}>
          {mode === null && <div className={styles.detecting} aria-busy="true"><span className="auth-spinner" /></div>}
          ...
          {mode === 'android-manual' && (
            <>
              <a className={styles.install} href="/">Open Zurvo</a>
            ...
            <>
              ...
              {iosOpen && (
                <ol className={styles.steps}>
                  <li>Tap the <b>Share</b> button <Share small /> in Safari’s toolbar</li>
                  <li>Scroll down and tap <b>Add to Home Screen</b></li>
                  <li>Tap <b>Add</b> — Zurvo lands on your home screen</li>
                </ol>
              )}
          ...
          {mode === 'desktop' && (
            <div className={styles.desktop}>
              ...
              <div>
                ...
                <p className={styles.desktopBody}>Zurvo is a mobile app. Scan the code, or visit this link on your Android or iPhone to install it.</p>
          ...
          {mode === 'installed' && (
            <>
              <a className={styles.install} href="/"><Check /> Open Zurvo</a>
      ...
      <section className={styles.features}>
        {FEATURES.map((f) => (
          <div key={f.title} className={styles.feature}>
            <span className={styles.fIcon} aria-hidden="true">{f.icon}</span>
            <div>
              <h2>{f.title}</h2>
              <p>{f.body}</p>
            </div>
      ...
      <footer className={styles.foot}>
        ...
        <a href="/">Already have an account? Open Zurvo →</a>
      </footer>
    </main>
  )
}
...
function Share({ small }) {
  ...
  return (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" aria-hidden="true" style={small ? { verticalAlign: 'middle' } : undefined}>
      ...
      <path d="M6 12H5a2 2 0 0 0-2 2v5a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-5a2 2 0 0 0-2-2h-1" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}
function Check() {
